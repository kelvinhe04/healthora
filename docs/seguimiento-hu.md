...
- **Correos transaccionales caen en spam** (Issue #171) — mitigación parcial mergeada en PR #176 (ver Bugs resueltos), pero la causa raíz sigue sin resolver: se envía correo vía SMTP de una cuenta Gmail personal (`healthora24@gmail.com`), sin dominio propio ni SPF/DKIM/DMARC. Requiere registrar un dominio (Cloudflare Registrar/Namecheap/Porkbun, ~$9-15/año) y migrar el envío a un servicio transaccional (Resend/SendGrid/SES/Postmark) con ese dominio verificado — es una compra/decisión del usuario, no algo que se resuelva solo con código. Issue se deja abierta hasta que eso se haga.
