...
| HU-038 / HU-100 | Costo de envío por zona + velocidad de entrega en checkout | `fix/hu-038-100-envio-zona-velocidad` / PR pendiente | Kelvin | Closes #42, Closes #185. `backend/src/lib/shipping.ts` (espejo en `frontend/src/lib/shipping.ts`, mismo patrón que `promotions.ts`) resuelve costo/etiqueta/ETA por zona (capital/interior/pickup) × velocidad (estándar/express); pickup siempre gratis, envío gratis sobre $50 se mantiene por zona. `Order` guarda `shippingZone`/`shippingSpeed`/`shippingLabel`/`shippingEta`; visible en `Orders.tsx` (cliente) y `OrdersSection.tsx` (admin). Selector nuevo en el paso 2 del checkout. Tests: `payment-flow.integration.test.ts` actualizado, e2e `checkout.spec.ts` verificado. |
