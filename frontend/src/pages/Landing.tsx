...
const NEEDS = [
  { id: 'Piel seca', label: 'Piel seca', emoji: '◐' },
  { id: 'Energía y vitaminas', label: 'Energía y vitaminas', emoji: '◑' },
  { id: 'Cuidado del bebé', label: 'Cuidado del bebé', emoji: '○' },
  { id: 'Fitness y recuperación', label: 'Fitness y recuperación', emoji: '●' },
];
  ...
  return (
    <main>
      ...
      <section style={{ padding: '32px 40px 0' }}>
        <div style={{ borderRadius: 32, overflow: 'hidden', background: 'var(--green)', color: 'var(--cream)', display: 'grid', gridTemplateColumns: '1.15fr 1fr', minHeight: 560, position: 'relative' }}>
          <div style={{ padding: '64px 72px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'relative', zIndex: 2 }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                ...
                <Button variant="outline" size="lg" style={{ color: 'var(--cream)', borderColor: 'rgba(255,255,255,0.22)' }}>Ver best sellers</Button>
              </div>
            </div>
          </div>
          <div style={{ position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 30% 30%, oklch(0.55 0.08 155) 0%, var(--green) 50%)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              {products[0] && <div style={{ position: 'absolute', top: 80, right: 60, transform: 'rotate(-8deg)' }}><ProductImage product={products[0]} size="md" /></div>}
              {products[2] && <div style={{ position: 'absolute', bottom: 110, right: 40, transform: 'rotate(6deg)' }}><ProductImage product={products[2]} size="sm" /></div>}
            </div>
          </div>
        </div>
      </section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
          {NEEDS.map((n) => (
            <div key={n.id} onClick={() => onNav('catalog', { need: n.id })} style={{ background: 'var(--cream-2)', borderRadius: 20, padding: '28px 24px', cursor: 'pointer', border: '1px solid var(--ink-06)', display: 'flex', flexDirection: 'column', gap: 40, minHeight: 200, transition: 'all 220ms' }}
              ...
              <div style={{ fontFamily: '"Instrument Serif", serif', fontSize: 44, lineHeight: 1, opacity: 0.6 }}>{n.emoji}</div>
              <div>
