...
export function productToForm(p: Product): ProductForm {
  return {
    ...
    sampleEligible: p.sampleEligible === true ? 'include' : p.sampleEligible === false ? 'exclude' : 'auto',
...
export function formToPayload(f: ProductForm): Partial<Product> {
  ...
  return {
    ...
    sampleEligible: f.sampleEligible === 'include' ? true : f.sampleEligible === 'exclude' ? false : null,
    benefits: f.benefits
      .split('\n')
      .map((b) => b.trim())
      .filter(Boolean),
