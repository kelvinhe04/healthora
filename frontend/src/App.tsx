...
function readStoredCatalogBrands(): string[] {
  try {
    const parsed = JSON.parse(
      sessionStorage.getItem(CATALOG_BRANDS_STORAGE_KEY) || "[]",
    );
...
function hasCatalogUrlFilters(searchParams: URLSearchParams): boolean {
  return ["category", "need", "search", "page", "brand"].some((key) =>
...
function readCatalogFilter(searchParams: URLSearchParams): CatalogFilter {
  ...
  const page =
    Number.isFinite(pageValue) && pageValue > 1 ? pageValue : undefined;
...
function normalizeCatalogFilter(filter?: CatalogFilter): CatalogFilter {
  ...
  return {
    category:
      filter?.category && filter.category !== "Todos"
        ? filter.category
        : undefined,
...
function AppInner() {
  ...
  const initialView =
    ...
    (localStorage.getItem("healthora_view") as View) ||
    "landing";
  const [view, setView] = useState<View>(initialView);
  const [catalogFilter, setCatalogFilter] = useState<CatalogFilter>(() =>
    readCatalogFilter(searchParams),
  );
